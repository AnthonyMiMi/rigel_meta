# Rigel_meta
The meta repository for Rigel

### ROS support
Rigel works with the following ROS verions
- jazzy
- humble
- noetic (legacy) 

# System overview
blah blah blah ...

# Developer notes
blah blah blah ...

# List of repositories
#### Hardware
- [rigel_hardware](https://github.com/AnthonyMiMi/rigel_hardware)

#### Meta packages
- [rigel_onboard_meta](https://github.com/AnthonyMiMi/rigel_onboard_meta)
- [rigel_basestation_meta](https://github.com/AnthonyMiMi/rigel_basestation_meta)
- [rigel_steamdeck_meta](https://github.com/AnthonyMiMi/rigel_steamdeck_meta)

#### Rigel specific ROS packages
- [rigel_ros](https://github.com/AnthonyMiMi/rigel_ros)
- [rigel_basestation_ros](https://github.com/AnthonyMiMi/rigel_basestation_ros)
- [rigel_steamdeck_ros](https://github.com/AnthonyMiMi/rigel_steamdeck_ros)
- [rigel_description](https://github.com/AnthonyMiMi/rigel_description)
- [rigel_msgs](https://github.com/AnthonyMiMi/rigel_msgs)
- [rigel_teleop](https://github.com/AnthonyMiMi/rigel_teleop)

#### Other ROS packages
- [inslam_ros](https://github.com/AnthonyMiMi/inslam_ros)